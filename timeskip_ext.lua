...
--------------------------------------------
Requires "timeskip_intf.lua" > Put the VLC Interface Lua script file in \lua\intf\ folder

Simple instructions:
1) "time_ext.lua" > Copy the VLC Extension Lua script file into \lua\extensions\ folder;
2) "timeskip_intf.lua" > Copy the VLC Interface Lua script file into \lua\intf\ folder;
...
vlc.exe --extraintf=luaintf --lua-intf=timeskip_intf
* VLC preferences for automatic activation of the Interface script:
Tools > Preferences > Show settings=All > Interface >
> Main interfaces: Extra interface modules [luaintf]
> Main interfaces > Lua: Lua interface [timeskip_intf]
...
config={}
local cfg={}
intf_script = "timeskip_intf" -- Location: \lua\intf\timeskip_intf.lua
-- defaults
...
(Extension script "time_ext.lua" + Interface script "timeskip_intf.lua")
...
function click_HELP()
	local help_text=[[
<div style="background-color:lightgreen;"><b>Time</b> is VLC Extension that displays running time on the screen in a playing video.</div>(Extension script "time_ext.lua" + Interface script "timeskip_intf.lua")
<hr />
...
function Get_config()
	local s = vlc.config.get("bookmark9")
...
function Set_config(cfg_table, cfg_title)
	...
	vlc.config.set("bookmark9", "config="..Serialize(config))
end
